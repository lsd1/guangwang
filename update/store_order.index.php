  ...
  <thead>
    <tr>
      ...
      <th class="w40"><?php echo $lang['store_show_order_amount'];?></th>
        <?php if($_GET['state_type'] == 'state_success'){?>
            <th class="w90">退款</th>
        <?php }?>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          <dt>
              ...
              <a target="_blank" class="blue ml5" href="<?php echo urlShop('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>">[交易快照]</a>
<!--              显示商品货号-->
            <?php $goods_serial = Model('goods')->getGoodsInfoByID($goods['goods_id'],'goods_serial');if ($goods_serial['goods_serial']){ ?>
                  <a target="_blank" class="blue ml5" href="javascript:;">[货号：<?php echo $goods_serial['goods_serial'];?>]</a>
            <?php } ?>
          </dt>
        ...
        <?php if($_GET['state_type'] == 'state_success'){?>
            <td><p>
                    <a href="javascript:void(0)"
                       class="ncbtn ncbtn-grapefruit mt5"
                       nc_type="dialog"
                       uri="index.php?act=member_refund&op=seller_add_refund&order_id=<?php echo $order['order_id']; ?>&goods_id=<?php echo $goods['rec_id'] ?>"
                       dialog_title="发起退款"
                       dialog_id="seller_add_refund"
                       dialog_width="400" id="order<?php echo $goods['rec_id']; ?>_action_refund" />
