 ...
 */
     ...
     *
     */
    public function seller_add_refundOp(){
        $model_refund = Model('refund_return');
        $condition = array();
        $reason_list = $model_refund->getReasonList($condition);//退款退货原因
        Tpl::output('reason_list',$reason_list);
        ...
        if ($refund['refund_id'] > 0) {//检查订单状态,防止页面刷新不及时造成数据错误
           echo '<h3 style="text-align: center;padding:50px;">已申请退款！</h3>';die;
        }

        if(!chksubmit()){
            Tpl::showpage('store_order.refund','null_layout');
        }elseif (chksubmit() && $goods_id > 0){
            $refund_array = array();
            $refund_amount = floatval($_POST['refund_amount']);//退款金额
            if (($refund_amount < 0) || ($refund_amount > $goods_pay_price)) {
                $refund_amount = $goods_pay_price;
            ...
            if ($state) {
               ...
               // showDialog(Language::get('wrong_argument'),'index.php?act=member_order&op=index','error');
                showDialog($message = '申请成功,请关闭对话框！', $url = '', $alert_type = 'succ', $extrajs = '', $time = 2);

            } else {
               // showDialog($message = '申请成功！', $url = '', $alert_type = 'succ', $extrajs = '', $time = 2);
                showDialog($message = '申请失败！', $url = '','error');
